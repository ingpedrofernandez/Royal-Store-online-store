<?php
		...
		if(isset($_POST['u_signup'])){
			...
			if($user_count==1){
				...
				if($add_user->execute()){
					...
					echo"<script>alert('Congo Registration Succcessfull Check Your Email We Send Password There')</script>";
		...
		while($row=$get_payment->fetch()):
			...
			echo"<tr>
					...
					if($status == 'Complete'){
						...
						echo"<td>Order Canclled</td>";
					}else{
						echo"<td><a href='user_pro.php?myorder&cancle=".$row['pay_id']."'>Cancle</a></td>";
					}
				echo"</tr>";
		endwhile;
		...
		if($cart_empty==0){
			...
			while($row=$get_cart_item->fetch()):
				...
				echo"<tr>
						...
						<td><a href='delete.php?user_delete_id=".$row_pro['pro_id']."'>Delete</a></td>
						<td>";
							$qty=$row['qty'];
		...
		if(!isset($_SESSION['u_email'])){
			...
			while($row=$get_cart_item->fetch()):
				...
				echo"<tr>
						<td><img src='imgs/pro_img/".$row_pro['pro_img1']."' /></td>
						<td>".$row_pro['pro_name']."</td>
						<td><input type='text' name='qty[".$row['cart_id']."]' value='".$row['qty']."' pattern='[1-3]{1}' /><input type='submit' name='up_qty' value='Save' /></td>
						<td>".$row_pro['pro_price']."</td>
			...
			echo"</table>";
					if(isset($_GET['checkout_cart'])){
						echo"<center><button id='buy_now'>
							...
							<button id='buy_now'>Net Total = $net_total</button></center>
							<form action='https://www.sandbox.paypal.com/cgi-bin/webscr' method='post'>

							  <!-- Identify your business so that you can collect the payments. -->
							  <input type='hidden' name='business' value='azazpatel399-facilitator@gmail.com'>
							  ...
							  <input style='width:20%; margin-left:80%; height:35px; border:1px solid #400040; margin-top:-35px; border-radius:3px' id='paypal' type='image' name='submit' border='0' src='imgs/paypal.png' alt='Buy Now'>
	...
	function electronics(){
		...
		$fetch_pro=$con->prepare("select * from products where cat_id='$cat_id' ORDER BY 1 DESC LIMIT 0,3");
		$fetch_pro->setFetchMode(PDO:: FETCH_ASSOC);
		$fetch_pro->execute();
			...
			echo"<div id='pro_img'>
				...
				<div id='pro_features'>
					...
					if($row_pro['pro_qty'] > 1){ 
						if($pro_dis > 0){
							echo"<h4>MRP Price : ".$row_pro['pro_price']."</h4>
							<h4 style='color:green; text-decoration:none'>Discount : ".$row_pro['pro_dis']."%</h4>
							<h4 style='color:green; text-decoration:none'>Total Saving : RS. $pro_sell_price</h4>";
						}
		...
		if(isset($_GET['checkout'])){
			...
			echo"<form method='post'>
					...
					<center>
						...
						<button id='buy_now' name='single_buy'>Cash On Delivery</button></form>
						<form action='https://www.sandbox.paypal.com/cgi-bin/webscr' method='post'>
						  ...
						  <!-- Display the payment button. -->
						  <input style='width:20%; height:35px; border:1px solid #400040; margin-top:20px; border-radius:3px' id='paypal' type='image' name='submit' border='0' src='imgs/paypal.png' alt='Buy Now'>
						  <img alt='' border='0' width='1' height='1' src='https://www.paypalobjects.com/en_US/i/scr/pixel.gif' >
						</form>
					</center>";
